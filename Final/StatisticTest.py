...
data_train = pd.read_csv("./data/month"+str(args.month)+".csv",dtype=dtype_list, low_memory=False)
...
target_numbers = np.zeros((1,len(target_cols)))
for index, label in enumerate(label_train):
	target_numbers += np.array(label)
target_numbers = target_numbers/sum(target_numbers[0]);
print target_numbers

last_instance_df = pd.read_csv("./data/train_ver2.csv", usecols=['ncodpers']+target_cols, dtype=dtype_list)
...
preds = np.argsort(target_numbers, axis=1)
preds = np.fliplr(preds)[0]
print 'preds', preds

test_id = np.array(pd.read_csv("./data/test_ver2.csv", usecols=['ncodpers'])['ncodpers'])
final_preds = []
for ind in range(len(test_id)):
	cust = test_id[ind]
	#print 'cust',cust
	top_products = target_cols[preds]
...
out_df.to_csv('sub_statistic.csv', index=False)
