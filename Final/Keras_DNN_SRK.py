...
'indrel_1mes'	: {-99.0:0, -99:0, "1.0":1, "1":1, "2.0":2, "2":2, "3.0":3, "3":3, "4.0":4, "4":4, 1.0:1, 1:1, 2.0:2, 2:2, 3.0:3, 3:3, 4.0:4, 4:4, 'P':5},
...
def batch_generator(file_name, batch_size, shuffle, train_input=True):
	while (True):
		if train_input:
			chunked_df = pd.read_csv(file_name, usecols=['ncodpers']+cols_to_use+numerical_cols+target_cols, chunksize=batch_size, low_memory=False)
		else:
			chunked_df = pd.read_csv(file_name, usecols=['ncodpers']+cols_to_use+numerical_cols, chunksize=batch_size, low_memory=False)
		nrows = 0
		for chunk_df in chunked_df:
			chunk_X = chunk_df[cols_to_use]
			chunk_X = chunk_X.fillna(-99)
			#chunk_X = chunk_X[chunk_X[cols_to_use+numerical_cols].isnull()==False]
			for col_ind, col in enumerate(cols_to_use):
				try:
					chunk_X[col] = chunk_X[col].apply(lambda x: mapping_dict[col][x])
				except:
					for i in range(len(chunk_X[col])):
						try:
							temp = mapping_dict[col][chunk_X[col][i]]
						except:
							print 'chunk_X[col][i]',chunk_X[col][i],'type',type(chunk_X[col][i])
							print 'Wrong!!! i',i
					print 'col', col
...
def keras_model():
	...
	final_model.add(Dense(512, input_dim=feat_count, init='he_uniform'))
	final_model.add(Activation('relu'))
	final_model.add(Dense(1024, input_dim=512, init='he_uniform'))
	final_model.add(Activation('relu'))
	final_model.add(Dense(512, input_dim=1024, init='he_uniform'))
	final_model.add(Activation('relu'))
	final_model.add(Dense(256, input_dim=512, init='he_uniform'))
	final_model.add(Activation('relu'))
...
if __name__ == "__main__":
	train = "./data/train_ver2.csv"
	test = "./data/test_ver2.csv"
	train_size = 13647309
	#train_size = 5000000
	test_size = 929615
	...
	fit= model.fit_generator(
			generator = batch_generator(train, 5000, True), 
			nb_epoch = 5,
	...
	out_df.to_csv('sub_keras_v2_512_1024_512_256.csv', index=False)